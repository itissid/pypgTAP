...
-- table_names_are_good( schema, project_prefix )
CREATE OR REPLACE FUNCTION table_names_are_good( NAME, NAME )
RETURNS TEXT AS $$
DECLARE
    output TEXT[];
    table_name     TEXT;
    table_type     TEXT    := 'BASE TABLE';
    allowed_pre_1  TEXT    := $2 || '_%';
    allowed_pre_2  TEXT    := 'f_' || $2 || '_%';
    allowed_pre_3  TEXT    := 'd_' || $2 || '_%';
BEGIN
    output := '{}';
    FOR table_name IN EXECUTE 'SELECT table_name::text
                                FROM information_schema.tables
                                WHERE table_type = ' || quote_literal(table_type) ||
                                  ' AND table_schema = ' || quote_literal($1) ||
                                  ' AND table_name NOT LIKE ' || quote_literal(allowed_pre_1) ||
                                  ' AND table_name NOT LIKE ' || quote_literal(allowed_pre_2) ||
                                  ' AND table_name NOT LIKE ' || quote_literal(allowed_pre_3) LOOP
